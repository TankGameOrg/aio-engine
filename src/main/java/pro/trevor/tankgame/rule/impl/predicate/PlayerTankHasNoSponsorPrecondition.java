...
public class PlayerTankHasNoSponsorPrecondition implements Precondition {
    @Override
    public Error test(State state, Player player) {
        ...
        if (maybeTank.get().has(Attribute.SPONSOR)) {
            return new Error(Error.Type.OTHER, "Tank already has a sponsor");
        }

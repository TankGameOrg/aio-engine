...
public class ShootFallen implements Action {
    ...
    public pro.trevor.tankgame.rule.action.Error apply(State state, LogEntry entry) {
        ...
        if (maybePosition.isEmpty()) {
            return new pro.trevor.tankgame.rule.action.Error(pro.trevor.tankgame.rule.action.Error.Type.OTHER, "Log entry does not contain a target position");
        }
        ...
        if (maybeUnit.isEmpty()) {
            return new pro.trevor.tankgame.rule.action.Error(pro.trevor.tankgame.rule.action.Error.Type.OTHER, "Target position is not on the game board");
        }

        LogEntry newEntry = new LogEntry();
        newEntry.put(Attribute.TARGET_POSITION, position);
        newEntry.put(Attribute.FALLEN_ACTION, ActionType.SHOOT);
